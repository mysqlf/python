...
def AES_encrypt(text, key, iv):
    ...
    tmp=pad * chr(pad)
    #因为类型问题，添加一个判断
    if type(tmp)==type(text):
        text = text + tmp
    else:
        text = text + tmp.encode("UTF-8")
...
def get_all_comments(url):
    ...
    encSecKey = get_encSecKey()
    json_text = get_json(url,params,encSecKey)
    #修改到这一步，发现我已经被网易ban了说我作弊
    
    if type(json_text) is not type("adb"):
        tmp=str(json_text, encoding = "utf-8")  
