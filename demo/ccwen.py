...
class BDTB:
    ...
    def __init__(self,baseUrl):
        ...
        self.defaultTitle = u"cc"+str(time.strftime('%Y-%m-%d',time.localtime(time.time())))+str(random.randint(10,20))
        #是否写入楼分隔符的标记
    ...
    def getContent(self,page):
        ...
        response = urllib.request.urlopen(request)
        #匹配所有楼层的内容
        pattern = re.compile('<div class="tpc_content do_not_catch.*?>(.*?)</div>',re.S)
    ...
    def writeData(self,contents):
        ...
        for item in contents:
            ...
            self.file.write(item.decode('utf-8','ignore'))#.decode('utf-8','ignore')
            self.floor += 1
...
print ("请输入帖子代号")
baseURL = 'http://og.90cl.org/htm_data/20/1605/' + str(input(u'http://og.90cl.org/htm_data/20/1605/'))
