...
STORM_SUPERVISOR_LOG_FILE = os.getenv(
    'STORM_SUPERVISOR_LOG_FILE', "supervisor.log")
...
def confvalue(name, extrapaths, daemon=True):
    ...
    command = [
        ...
        "-cp", get_classpath(extrapaths,
                             daemon), "org.apache.storm.command.ConfigValue", name
    ]
    p = sub.Popen(command, stdout=sub.PIPE)
    output, errors = p.communicate()
    # python 3
...
def resolve_dependencies(artifacts, artifact_repositories):
    ...
    print("Resolving dependencies on demand: artifacts (%s) with repositories (%s)" % (
    ...
    if p.returncode != 0:
        raise RuntimeError(
            "dependency handler returns non-zero code: code<%s> syserr<%s>" % (p.returncode, errors))
    ...
    try:
        ...
        raise RuntimeError(
            "dependency handler returns non-json response: sysout<%s>", output)
...
def jar(jarfile, klass, *args):
    ...
    artifact_to_file_jars = resolve_dependencies(
        DEP_ARTIFACTS_OPTS, DEP_ARTIFACTS_REPOSITORIES_OPTS)

    transform_class = confvalue(
        "client.jartransformer.class", [CLUSTER_CONF_DIR])
    if (transform_class != None and transform_class != "null"):
        tmpjar = os.path.join(tempfile.gettempdir(), uuid.uuid1().hex + ".jar")
        exec_storm_class("org.apache.storm.daemon.ClientJarTransformerRunner", args=[
                         transform_class, jarfile, tmpjar], fork=True, daemon=False)
...
def sql(sql_file, topology_name):
    ...
    sql_runtime_jars = get_jars_full(
        STORM_DIR + "/external/sql/storm-sql-runtime")
...
def nimbus(klass="org.apache.storm.daemon.nimbus.Nimbus"):
    ...
    jvmopts = parse_args(confvalue("nimbus.childopts", cppaths)) + [
        ...
        os.path.join(get_log4j2_conf_dir(), "cluster.xml"),
    ]
    exec_storm_class(
        klass,
        jvmtype="-server",
...
def main():
    ...
    config_list, jars_list, artifacts_list, artifact_repositories_list, args = parse_config_opts(sys.argv[
                                                                                                 1:])
